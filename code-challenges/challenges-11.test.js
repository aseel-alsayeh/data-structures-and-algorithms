...
function transformToLis(obj){
  ...
  for(let property in obj){
    arr.push(`<li>${property}: ${obj[property]}</li>`)
  }
  return arr
}
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  // let inputArray=input.flat()
  // return inputArray.map(a=>{
  //   if (typeOf (a)!=='number' && a%5 !== 0){
  //    return a
  //   }
  // }).arrFun1.map(a=>{
  //   return Math.power(2,a)
  // })
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  return data.filter((e) => e.gender === 'male' || e.gender === 'female')
...
let findShortest = (data) => {
  // Solution code here...
  let height = data.reduce((a,b) => (Number(a.height) < Number(b.height) ? a : b));
  return height.name;
};
