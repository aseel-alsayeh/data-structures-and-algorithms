...
const toLastNames = people => {
  ...
  let list=[]
  people.map(e=>{
    return list.push(e.firstName + ' '+ e.lastName)
  })
  return list
};
...
const addPurchases = (arr) => {
  ...
  let purchasePrices=[];
  arr.map(e=>{
    return purchasePrices.push(e.purchasePrice)
  })
  let sum = purchasePrices.reduce( function(accumulator,value,idx) {
    accumulator = accumulator + value;
    return accumulator;
  }, 0);
  return sum
...
const returnNames = (arr) => {
  // Solution code here...
  let folks = arr.reduce((person, element) => {
     person.push(element.name)
    return person;
  }, [] );
  return folks
};
